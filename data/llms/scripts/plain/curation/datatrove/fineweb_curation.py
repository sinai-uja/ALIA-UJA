...
def main(cfg):
    ...
    INPUT_PATH = cfg["INPUT_PATH"].format(name=NAME, domain=DOMAIN) if VERSION == -1 else cfg["INPUT_PATH_VERSION"].format(name=NAME, domain=DOMAIN, version=VERSION)
    OUTPUT_PATH = cfg["OUTPUT_PATH"].format(name=NAME, domain=DOMAIN) if VERSION == -1 else cfg["OUTPUT_PATH_VERSION"].format(name=NAME, domain=DOMAIN, version=VERSION)
    
    # 📁 Rutas

    print(f"Configuración cargada: {cfg}")
    print(f"Directorio entrada: {INPUT_PATH}\nDirectorio salida: {OUTPUT_PATH}")

    os.makedirs(OUTPUT_PATH, exist_ok=True)

    # 📄 Cargar configuración para español desde YAML

    # 🧪 Paso 1: lector + idioma
    step_1_output = os.path.join(OUTPUT_PATH, "01.es")
    pipeline_1 = [
        JsonlReader(INPUT_PATH, glob_pattern="*.jsonl"),
        LanguageFilter(
            language_threshold=cfg["language_score"],
            languages=("es",),  # 🇪🇸 español
            exclusion_writer=JsonlWriter(os.path.join(OUTPUT_PATH, "01.filtered-out"))
        ),
        JsonlWriter(step_1_output)
    ]

    LocalPipelineExecutor(
        pipeline=pipeline_1,
        tasks=25,
        logging_dir=os.path.join(OUTPUT_PATH, "logs/01_language")
    ).run()


    # 🧹 Paso 2: deduplicado con MinHash
    sig_path = os.path.join(OUTPUT_PATH, "minhash", "signatures")
    buckets_path = os.path.join(OUTPUT_PATH, "minhash", "buckets")
    remove_ids = os.path.join(OUTPUT_PATH, "minhash", "remove_ids")
    dedup_output = os.path.join(OUTPUT_PATH, "02.deduplicated")

    minhash_cfg = MinhashConfig(
        hash_config=HashConfig(hash_fc="xxhash", precision=64),
        num_buckets=25,
        hashes_per_bucket=12,
        n_grams=5,
    )

    # Step 2.1 – Firma MinHash
    LocalPipelineExecutor([
        JsonlReader(step_1_output),
        MinhashDedupSignature(output_folder=sig_path, config=minhash_cfg, language="es"),
    ],  tasks=25, logging_dir=os.path.join(OUTPUT_PATH, "logs/mh1")).run()

    # Step 2.2 – Buckets
    LocalPipelineExecutor([
        MinhashDedupBuckets(input_folder=sig_path, output_folder=buckets_path, config=minhash_cfg)
    ],  tasks=25, logging_dir=os.path.join(OUTPUT_PATH, "logs/mh2")).run()

    # Step 2.3 – Cluster
    LocalPipelineExecutor([
        MinhashDedupCluster(input_folder=buckets_path, output_folder=remove_ids, config=minhash_cfg)
    ], tasks=1, logging_dir=os.path.join(OUTPUT_PATH, "logs/mh3")).run()

    # Step 2.4 – Filtro final
    LocalPipelineExecutor([
        JsonlReader(step_1_output),
        MinhashDedupFilter(input_folder=remove_ids),
        JsonlWriter(dedup_output)
    ],  tasks=25, logging_dir=os.path.join(OUTPUT_PATH, "logs/mh4")).run()


    # 🧪 Paso 3: filtros de calidad + salida final
    final_output = os.path.join(OUTPUT_PATH, "03.cleaned")

    pipeline_3 = [
        JsonlReader(dedup_output),
        ...
        GopherRepetitionFilter(
            ...
            exclusion_writer=JsonlWriter(os.path.join(OUTPUT_PATH, "03.removed_gopherrep"))
        ),
        ...
        FineWebQualityFilter(
            ...
            exclusion_writer=JsonlWriter(os.path.join(OUTPUT_PATH, "03.removed_fwqlty"))
        ),
        ...
        GopherQualityFilter(
            ...
            min_stop_words=2,
            exclusion_writer=JsonlWriter(os.path.join(OUTPUT_PATH, "03.removed_gopherqual"))
        ),
    ...
    LocalPipelineExecutor(
        pipeline=pipeline_3,
        tasks=25,
        logging_dir=os.path.join(OUTPUT_PATH, "logs/final")
    ).run()

if __name__ == "__main__":
    import multiprocessing
    multiprocessing.set_start_method("forkserver", force=True)
    with open(f"{os.path.dirname(os.path.realpath(__file__))}/config_es.yaml", "r") as f:
