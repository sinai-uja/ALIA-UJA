""""Script de creación del dataset CuadernosDeArtePrehistorico.
"""
...
class Scraper:
    def __init__(self, config_path: str, folders: list[str], urls: dict) -> None:
        ...
        Args:
            ...
            urls: Diccionario de enlaces sobre las que aplicar web scraping.   
        """
    ...
    def is_direct_pdf(self, url: str) -> bool:
        try:
            headers = {
            ...
            self.logger.warning(f"No se pudo verificar si la URL es PDF ({url}): {e}")
            return False

    async def extract_pdf_from_page(self, page_url: str, base_name: str):
        try:
            async with async_playwright() as p:
                ...
                # Esperar algún enlace que apunte a PDF o contenga "download"
                enlace = await page.query_selector("a[href$='.pdf'], a[href*='download']")
                if enlace:
                    ...
                    if href:
                        ...
                        self.download_pdf(pdf_url, base_name)
                    else:
                        print(f"No se pudo extraer href desde la página {page_url}")
                else:
                    print(f"No se encontró enlace de descarga en la página {page_url}")
                await browser.close()
        except Exception as e:
            self.logger.error(f"Error al extraer PDF desde página {page_url}: {e}")
            ...
            for record in records:
                ...
                for i, rel in enumerate(relation):
                    if rel.startswith('http'):
                        pdf_name = base_name
                        if self.is_direct_pdf(rel):
                            self.download_pdf(rel, pdf_name)
                        else:
                            await self.extract_pdf_from_page(rel, pdf_name)
...
if __name__ == "__main__":
    ...
    scraper = Scraper(
        ...
        folders=["ALIA", "Patrimonio_Cuadernos_De_Arte_Prehistorico"],
        urls="https://www.revistacuadernosdearteprehistorico.com/index.php/cdap/oai" 
    )
