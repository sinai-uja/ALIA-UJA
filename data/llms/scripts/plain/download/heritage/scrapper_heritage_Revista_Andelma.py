"""Recolector de datos OAI-PMH para la Revista Andelma.

Este módulo recolecta artículos de la Revista Andelma (publicación de
folklore y cultura popular) mediante OAI-PMH utilizando el cliente Sickle.

Example:
    python scrapper_heritage_Revista_Andelma.py

Note:
    URL: https://www.revistaandelma.es/index.php/andelma/oai
"""
...
class Scraper:
    ...
    Example:
        >>> Scraper("config.yaml", ["ALIA", "Revista_Andelma"],
        ...         "https://www.revistaandelma.es/index.php/andelma/oai").execute()
    """
    ...
    def download_file(self, url: str, folder: str, name: str, ext: str) -> tuple[str, str]:
        """Download and save file. Returns tuple (full_path, filename_without_ext) or ("", "") if failed."""
        try:
            headers = {
    ...
    def extract_text_from_pdf(self, pdf_path: str) -> str:
        try:
            ...
            # Primer intento: extracción normal
            for page in pdf_reader.pages:
                page_text = page.extract_text()
                if page_text:
                    extracted_text += page_text + "\n"

            # Si el texto está vacío, intentar OCR
    ...
    async def extract_pdf_from_page(self, page_url: str, base_name: str) -> tuple[str, str]:
        """Returns tuple (text_content, actual_filename_used)"""
        try:
                ...
                if enlace:
                    ...
                    if href:
                        ...
                        if is_doc:
                            ...
                            ext = doc_type
                            path, actual_filename = self.download_file(pdf_url, folder_map[ext], base_name, ext)
                            if not path:
                                return "", ""
                            if ext == "pdf":
    ...
    async def process_registers(self) -> None:
        ...
        try:
            ...
            for record in records:
                ...
                title = sanitize_filename(metadata.get('title', [])[0] or [])
                if len(title) > 100:
                ...
                pdf_text = ""
                actual_id = title  # Default to title, will be updated with actual filename if file is downloaded
                for rel in relation:
                    if pdf_text == "":
                        if rel.startswith('http'):
                            ...
                            if is_doc:
                                folder_map = {"pdf": self.pdf_folder, "epub": self.epub_folder, "docx": self.docx_folder}
                                path, actual_filename = self.download_file(rel, folder_map[doc_type], title, doc_type)
                                if not path:
                                    continue
                                
                                # Update actual_id with the filename that was actually used
                                ...
                                pdf_text, actual_filename = await self.extract_pdf_from_page(rel, title)
                                if actual_filename:
                                    actual_id = actual_filename
...
if __name__ == "__main__":
    scraper = Scraper(
        ...
        folders=["ALIA", "Revista_Andelma"],
        urls="https://www.revistaandelma.es/index.php/andelma/oai"
    )
    scraper.execute()