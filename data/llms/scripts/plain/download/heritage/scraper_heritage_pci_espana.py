"""Recolector de datos para PCI Nacional de España.

Este módulo implementa un método que recolecta información sobre
el Patrimonio Cultural Inmaterial (PCI) Nacional de España.
Los datos se extraen del portal del Ministerio de Cultura.

El portal contiene:
    - Manifestaciones culturales declaradas PCI Nacional
    - Descripciones, galerías de imágenes
    - Patrimonio inmaterial representativo de España

Example:
    Ejecución básica::

        python scraper_heritage_pci_espana.py

    Esto navegará por el portal, extraerá información
    y generará un archivo Parquet con metadatos.

Attributes:
    PORTAL_URL (str): URL principal del portal PCI Nacional.

Note:
    Los datos son de acceso público.
    URL: https://www.portalinmaterial.cultura.gob.es/pci-nacional.html
"""
...
# URL del portal PCI Nacional
PORTAL_URL = "https://www.portalinmaterial.cultura.gob.es/pci-nacional.html"
...
class DataCollector:
    ...
    desde el Portal del Patrimonio Inmaterial Nacional.
    ...
    async def collect_pci_nacional(self) -> dict:
        """Recopila elementos del PCI Nacional.
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Obtener lista de elementos PCI
                link_els = await page.query_selector_all("p.titulo a")
                items_data = []
                ...
                for i, (title, href) in enumerate(items_data):
                    item_id = f"PCI_Nacional_{i}"

                    if item_id in self.existing_ids:
                        continue

                    try:
                        self.logger.info(f"[PCI] Procesando: {title[:50]}...")
                        await page.goto(href, timeout=30000)

                        # Extraer texto
                        text_items = await page.query_selector_all("div.cte p.ta-justify")
                        text = "\n".join([
                            (await t.text_content()).strip() for t in text_items
                        ])
                        ...
                        self.logger.warning(f"[Timeout] {href}")
                        stats["errors"] += 1
                    except Exception as e:
    ...
    def execute(self) -> None:
        ...
        self.logger.info("=" * 60)
        self.logger.info("INICIANDO RECOLECCIÓN DE PCI NACIONAL")
        self.logger.info("=" * 60)

        stats = asyncio.run(self.collect_pci_nacional())
