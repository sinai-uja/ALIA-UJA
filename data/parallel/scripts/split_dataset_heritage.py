...
# Configure logging
log_path = 'logs/split_del_dataset_de_patrimonio.log'
...
input_path = 'data/raw/heritage/'
output_path = 'data/proccessed/heritage/'
dataset_identifiers = {
    'PCI': '01',
}
...
for file in tqdm(parquet_files, desc="Processing files"):
    try:
        ...
        num_rows = len(df)
        apply_transform = [random.random() < 10.0 for _ in range(num_rows)]
        ...
        # Apply transformation conditionally to both columns using native Polars expressions
        df = df.with_columns([
...
if not processed_dfs:
    ...
    train_df = full_df.slice(0, train_size)
    val_df = full_df.slice(train_size, val_size)
    test_df = full_df.slice(train_size + val_size)
    ...
    print("Dataset split and saved successfully using polars. Check the log file for details: logs/split_del_dataset_de_patrimonio.log")