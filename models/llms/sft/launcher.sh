#!/bin/bash
#SBATCH --job-name=ALIA_train_carbon_sft
...
#SBATCH --mem=0
#SBATCH --output="logs/%A_ALIA_train_carbon_sft.out"
#SBATCH --error="logs/%A_ALIA_train_carbon_sft.err"
...
srun torchrun \
  ...
  --rdzv_id=alia_run_sft_$SLURM_JOBID \
  "$TRACKER_SCRIPT" \
  --config "$CONFIG_FILE" \
