...
The methodology in this work adapts this to hate speech classification: instead of extracting final-layer embeddings from LLM experts (which is computationally expensive), we use both the comment embeddings and the embeddings of the explanation texts generated by each LLM to represent its reasoning semantically. The fuser input vector can concatenate up to three levels:
...
- Embeddings are cached to disk in the configuration-specific path
...
The results below correspond to the last experiment runs over the test partitions of each dataset. They are ordered from the simplest baselines to the fusion experiments: individual LLMs, majority vote, and then FoE. The individual-model and majority-vote blocks serve as the first comparison point before introducing fusion.
...
The following subsections present the FoE-based results for reference, positioned after the non-fusion baselines.
